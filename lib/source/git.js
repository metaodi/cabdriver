...
class Git extends Source {
    constructor(options, auth, gitlog, gitconfig) {
        super(options, auth);
        this.type = 'git';
        this.gitlog = gitlog || gitlogPromise;
            ...
            .then(function(results) {
                ...
                return Promise.all(
                    _.map(repoPaths, function(repoPath) {
                        return me.getCommitsFromRepository(
                            gitConfig,
                            repoPath,
                            me.options.startDate,
                            me.options.endDate,
                            me.options.verbose
                        );
                    })
    ...
    getCommitsFromRepository(config, repoPath) {
        ...
        return me.gitlog(options)
            ...
            .catch(function(err) {
                var msg = err.msg || err;
                if (msg && msg.match(/your current branch '.*' does not have any commits yet/)) {
