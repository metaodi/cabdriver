...
class Gitlab extends Source {
                ...
                return Async.map(events, function(entry, entryCb) {
                    ...
                    if (entry.target_id) {
                        if (entry.target_id in targets) {
                            ...
                            target = me.getGitlabTarget(auth, entry.target_type, entry.target_iid, entry.project_id, entry)
    ...
    eventDescription(entry, target) {
        ...
        if (target) {
            ...
            if (entry.push_data && entry.push_data.ref_type) {
                text += entry.push_data.ref_type + ' ';
            }
            if (entry.push_data && entry.push_data.ref) {
