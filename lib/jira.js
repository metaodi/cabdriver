        ...
        .then(function(result) {
            Async.map(
                ...
                function(issue, issueCb) {
                    //TODO: check for proper solution with https://github.com/jira-node/node-jira-client/pull/104
                    jira.findIssue(issue.key + '?expand=changelog')
                        .then(generateDailyEntries(currentUser, startDateObj, endDateObj, issueCb))
                        .catch(issueCb);
                },
...
function generateDailyEntries(currentUser, startDate, endDate, callback) {
    return function(issue) {
        ...
        var myChangelogEntries = _.filter(issue.changelog.histories, function(entry) {
            ...
            return entry.author.key === currentUser && created.isBetween(startDate, endDate, null, '[]');
        });
        ...
        var myWorklog = _.filter(issue.fields.worklog.worklogs, function(log) {
            ...
            return log.author.key === currentUser && started.isBetween(startDate, endDate, null, '[]');
        });
