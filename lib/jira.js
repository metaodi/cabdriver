...
Jira.prototype.getActivities = function(callback, auth, options) {
    var me = this;
    var config = {
        protocol: 'https',
        host: 'jira.liip.ch',
            ...
            Async.map(
                ...
                function(issue, issueCb) {
                    jira.findIssue(issue.key, 'changelog', '*all', '*')
                        .then(me.generateDailyEntries(currentUser, startDateObj, endDateObj, options.verbose, issueCb))
                        .catch(function(err) {
                            if (err.message) {
                                err = err.message;
                            }
                            helper.errorVerbose('Error fetching issue ' + issue.key + ': ' + err, options.verbose);
                            issueCb(null, []);
                        });
                },
