...
function getSources(options, override) {
    var sources = {
        calendar: new GoogleCalendar(new GoogleAuth(), options),
        mail: new GoogleMail(new GoogleAuth(), options),
        gitlab: new Gitlab(new GitlabAuth(), options),
        jira: new Jira(new JiraAuth(), options),
        github: new Github(new GithubAuth(), options)
    };
