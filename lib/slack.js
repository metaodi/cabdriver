...
Slack.prototype.dailyStats = function(callback, auth, options) {
    var start = Moment.tz(options.startDate, 'Europe/Zurich').subtract(1, 'd');
    var end = Moment.tz(options.endDate, 'Europe/Zurich').add(1, 'd');
    var searchArgs = {
            'token': auth,
            'sort': 'timestamp',
            'sort_dir': 'asc',
    ...
    SlackApi.search.messages(searchArgs, function(err, data) {
        if (err) {
        ...
        Async.mapValues(messagesByDay, function(dayMsgs, dayTimestamp, dayCb) {
            ...
            Async.mapValues(
                ...
                function(msgs, group, groupCb) {
                    if (group[0] === 'C') {
                        SlackApi.channels.info({'token': auth, 'channel': group}, function(err, channelData) {
                            if (err) {
                                groupCb(err);
                                return;
                            }
                        ...
                        SlackApi.users.info({'token': auth, 'user': group}, function(err, userData) {
                            var msg = {
                                'project': 'xxx',
                                'time': '',
